...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.expand_widget.setText("图片局部放大")
        self.expand_widget.setFixedSize(200, 200)
        self.expand_widget.move(160, 160)

        self.expand_dock = QtWidgets.QDockWidget('放大镜', self)
        ...
        self.shape_dock = QtWidgets.QDockWidget('标注结果', self)
        ...
        self.file_dock = QtWidgets.QDockWidget(u'文件列表', self)
        ...
        editMode = action('编辑', self.setEditMode,
                          shortcuts['edit_polygon'], 'edit',
                          '移动和编辑标签', enabled=False)

        delete = action('删除', self.deleteSelectedShape,
                        shortcuts['delete_polygon'], 'cancel',
                        '删除', enabled=False)
        copy = action('复制', self.copySelectedShape,
        ...
        self.menus = utils.struct(
            ...
            view=self.menu('&视图'),
            help=self.menu('&帮助'),
            recentFiles=QtWidgets.QMenu('历史 &文件'),
            labelList=labelMenu,
        )
    ...
    def changeOutputDirDialog(self, _value=False):
        ...
        output_dir = QtWidgets.QFileDialog.getExistingDirectory(
            self, '%s - 在目录中保存/加载注释' % __appname__,
            default_output_dir,
    ...
    def deleteFile(self):
        ...
        if osp.exists(label_file):
            ...
            logger.info('删除标签文件: {}'.format(label_file))
    ...
    def mayContinue(self):
        if not self.dirty:
            return True
        mb = QtWidgets.QMessageBox
        msg = '在关闭之前将注释保存到 "{}"?'.format(self.filename)
    ...
    def chshapeLineColor(self):
        color = self.colorDialog.getColor(
            self.lineColor, '选择线的颜色', default=DEFAULT_LINE_COLOR)
        if color:
    ...
    def openDirDialog(self, _value=False, dirpath=None):
        ...
        targetDirPath = str(QtWidgets.QFileDialog.getExistingDirectory(
            self, '%s - 开目录' % __appname__, defaultOpenDirPath,
                  QtWidgets.QFileDialog.ShowDirsOnly |
                  QtWidgets.QFileDialog.DontResolveSymlinks))
